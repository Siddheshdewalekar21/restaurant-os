...
io.on('connection', (socket) => {
    ...
    authSocket.on('order:status:update', async (data, callback) => {
        var _a;
        try {
            ...
            if (status === 'PREPARING') {
                const kitchenTicket = {
                    ...
                    tableNumber: (_a = updatedOrder.table) === null || _a === void 0 ? void 0 : _a.tableNumber,
    ...
    authSocket.on('table:status:update', async (data, callback) => {
        try {
            ...
            // Send error if callback exists
            if (callback)
                callback({ success: false, error: 'Failed to update table status' });
        }
    });
});
// Start the server
